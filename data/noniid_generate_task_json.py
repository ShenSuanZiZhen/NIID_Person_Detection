...
if model == "faster":
    ...
    for i in range(1, 6):
        ...
        task_config["data_path"] = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/images/camera_{}".format(str(i))
        task_config["local_epoch"] = 3
    ...
    with open(model_config_file, 'w') as f:
        json.dump(model_config, f, indent=4)
    for i in range(1, 6):
        label_path = os.path.join("/home/ykn/dataset/NonIID_Person_Detect_Dataset/yolo_label","camera_"+str(i))
        if not os.path.exists(label_path):
            os.mkdir(label_path)
        for image_set in sets:
            path1 = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/voc_label"
            path2 = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/list_file"
            # anno_path = os.path.join(dataset, str(i), "Annotations")
            # image_path = os.path.join(dataset, str(i), "ImageSets", "Main", image_set + ".txt")
            anno_path = os.path.join(path1, "camera_" + str(i))
            image_path = os.path.join(path2, "camera_" + str(i), image_set + ".txt")
            
            image_ids = open(image_path).read().strip().split()

            for image_id in image_ids:
                convert_annotation(anno_path, label_path, image_id[-8:-4])
           # list_file.close()
        ...
        task_config["log_filename"] = "{}/{}/FL_client_{}_log".format(model, dataset, str(i))
        task_config["train"] = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/list_file/camera_{}/train.txt".format(str(i))
        task_config["test"] = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/list_file/camera_{}/test.txt".format(str(i))
        task_config["names"] = "/home/ykn/dataset/NonIID_Person_Detect_Dataset/list_file/classes.names"
