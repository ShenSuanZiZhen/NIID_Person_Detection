...
if model == "faster":
    ...
    with open(model_config_file, 'w') as f:
        json.dump(model_config, f, indent=4)
    for i in range(1, 6):
        label_path = os.path.join("/home/ykn/dataset/Street_Dataset/yolo_label",str(i))
        if not os.path.exists(label_path):
            os.mkdir(label_path)
        for image_set in sets:
            path1 = "/home/ykn/dataset/Street_Dataset/device_Annotations"
            path2 = "/home/ykn/dataset/Street_Dataset/device_ImageSets"
            # anno_path = os.path.join(dataset, str(i), "Annotations")
            # image_path = os.path.join(dataset, str(i), "ImageSets", "Main", image_set + ".txt")
            anno_path = os.path.join(path1, str(i), "Annotations")
            image_path = os.path.join(path2,"Main", str(i), image_set + ".txt")
            
            image_ids = open(image_path).read().strip().split()
            list_file = open('%s/%s/%s.txt' % ("/home/ykn/dataset/Street_Dataset/list_file/street_5", str(i), image_set), 'w')
        ...
        task_config["train"] = "/home/ykn/dataset/Street_Dataset/list_file/{}/{}/train.txt".format(dataset, str(i))
        task_config["test"] = "/home/ykn/dataset/Street_Dataset/list_file/{}/{}/test.txt".format(dataset, str(i))
        task_config["names"] = "/home/ykn/dataset/Street_Dataset/list_file/{}/classes.names".format(dataset)
