...
class FederatedClient(object):
    ...
    def register_handles(self):
        ...
        def on_request_update(*args):
            ...
            #ykn add 2022.9.12 save temp model
            model_name =  self.local_model.model_name
            temp_weights_file_path =  os.path.join("temp_model", model_name,"camera_"+str(self.id)+".pkl")
            if  not os.path.exists(temp_weights_file_path):
                os.makedirs(os.path.dirname(temp_weights_file_path), exist_ok=True)
            self.logger.info("save camera_{} model to pkl file".format(self.id))
            output = open(temp_weights_file_path, 'wb')
            pickle.dump(my_weights, output)
            #end

            pickle_string_weights = obj_to_pickle_string(my_weights)
            resp = {
                'round_number': cur_round,
                # 'weights': pickle_string_weights, #not stable
                'weights': temp_weights_file_path, #not stable
                'train_size': self.local_model.model.train_size,
                'train_loss': train_loss,
                'temp_weights_file_path': temp_weights_file_path #stable
            }
...
if __name__ == "__main__":
    ...
    print("client id {}".format(opt.config_file[-6]))
    try:
        FederatedClient("127.0.0.1", opt.port, opt.config_file, opt.gpu, opt.ignore_load,int(opt.config_file[-6]))
