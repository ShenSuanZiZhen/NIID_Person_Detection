...
class Aggregator(object):
    ...
    # # client_updates = [(w, n)..]
    # def update_weights(self, client_weights_path, client_sizes):
    #     client_weights = []
    #     self.logger.info("start update weights")
    #     total_size = np.sum(client_sizes)
    #     #self.logger.info(total_size)
    #     for l in range(len(client_weights_path)):
    #         client_weights[l] = pickle_string_to_obj(client_weights_path[l])
    #     new_weights = [np.zeros(param.shape) for param in client_weights[0]]
    #     self.logger.info(len(client_weights))
    #     for c in range(len(client_weights)):
    #         self.logger.info(c)
    #         for i in range(len(new_weights)):
    #             #self.logger.info("len(new_weights): {}".format(i))
    #             new_weights[i] += (client_weights[c][i] * client_sizes[c]
    #                                / total_size)
    #     self.current_weights = new_weights
    #     self.logger.info("finish update weights")
    #     # return new_weights

    def update_weights(self, client_weights, client_sizes):
        total_size = np.sum(client_sizes)
        #self.logger.info(client_weights)
        new_weights = [np.zeros(param.shape) for param in client_weights[0]]
        # self.logger.info(new_weights)
        for c in range(len(client_weights)):
            # self.logger.info(c)
            for i in range(len(new_weights)):
                new_weights[i] += (client_weights[c][i] * client_sizes[c]
    ...
    def aggregate_train_loss_accuracy_recall(self, client_losses, client_sizes, cur_round):
        ...
        self.logger.info("=== aggregate train loss accuracy recall ===")
        return aggr_loss
    ...
    def register_handles(self):
        ...
        def handle_client_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                #stable model agg ykn 2022.9.12
                self.current_round_client_updates[-1]['weights'] = pickle_string_to_obj(data['temp_weights_file_path'])
                #self.logger.info(self.current_round_client_updates[-1]['weights'])
